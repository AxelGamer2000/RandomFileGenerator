import cmd, orjson, pathlib, shutil, code, importlib
...
Syntaxe: find <ext>
    ...
    def do_delall(self, arg):
        """
Delete all file and directory in generation directory or certain files with selected ext
Syntaxe: delall <ext> / delall
        """
        if arg == "":
            ...
            for dirpath, dirname, filename in file_list:
                for i, e in enumerate(filename):
                    if e.endswith(arg):
                        pathlib.Path(dirpath/e).unlink(missing_ok=True)
                        print(f"{e} deleted")
...
Syntaxe: count
        ...
        if count <= 1:
            print(str(count) + " file with the ext " + arg)
        else:
            print(str(count) + " files with the ext " + arg)

    def do_run(self, arg):
        """
Run the rfg file in the directory scripts
Syntaxe: run <filename>
        """
        runScript(pathlib.Path(arg + ".rfg"))

    def do_console(self, args):
        """
 Run the rfg console like the basic python console with rfg python integrated
 Syntaxe: console
        """
        rfg = importlib.import_module("rfg_python")
        local_env = rfg.__dict__.copy()
        code.interact(local=local_env, banner="Welcome to RFG Console ! \nWrite help() for the help.")
...
Syntaxe: home
        """
        exec(pathlib.Path("launcher.py").read_text())
